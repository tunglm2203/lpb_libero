...
def main(args):
    ...
    for task in all_tasks:
        ...
        all_scores_across_tasks.append(np.mean(all_scores) * 100)

        cprint(f"Task: {task}: mean_score_across_seeds={np.mean(all_scores) * 100:.2f}, std={np.std(all_scores) * 100:.2f} ", 
              "green", attrs=["bold"])
