...
def axisangle2quat_batch(vecs):
    angles = np.linalg.norm(vecs, axis=1, keepdims=True)  # (n, 1)

    safe_angles = np.where(angles == 0, 1, angles)
    axes = vecs / safe_angles

    half_angles = angles / 2.0
    sin_half_angles = np.sin(half_angles)  # (n, 1)
    cos_half_angles = np.cos(half_angles)  # (n, 1)

    quat_xyz = axes * sin_half_angles  # (n, 3)
    quat_w = cos_half_angles  # (n, 1)

    quats = np.concatenate([quat_xyz, quat_w], axis=1)  # (n, 4)

    is_zero = np.isclose(angles.flatten(), 0.0)
