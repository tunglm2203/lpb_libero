...
from diffusion_policy.policy.reflow_transformer_hybrid_image_policy import ReFlowTransformerHybridImagePolicy
...
# %%
class TrainReFlowTransformerHybridWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        # configure model
        self.model: ReFlowTransformerHybridImagePolicy
        ...
        self.ema_model: ReFlowTransformerHybridImagePolicy = None
        if cfg.training.use_ema:
            self.ema_model = copy.deepcopy(self.model)

        # configure training state
        # unlike the lowdim policy, the obs encoder needs its own weight decay
        # group, so the optimizer is built by the policy itself.
        ...
        for local_epoch_idx in range(cfg.training.num_epochs):
            ...
            with tqdm.tqdm(train_dataloader, desc=f"Training epoch {self.epoch}", leave=False, mininterval=cfg.training.tqdm_interval_sec) as tepoch:
                for batch_idx, batch in enumerate(tepoch):
                    ...
                    # update ema
                    if cfg.training.use_ema and self.global_step % self.update_ema_freq == 0:
                        self.step_ema()
            ...
            if (self.epoch % cfg.training.sample_every) == 0:
                with torch.no_grad():
                    ...
                    obs_dict = batch['obs']
                    # the sampled trajectory starts at the current step, see
                    # ReFlowTransformerHybridImagePolicy.compute_loss
            ...
            if self.epoch != 0 and ((self.epoch % cfg.training.checkpoint_every) == 0 or self.epoch == cfg.training.num_epochs - 1):
                ...
                # keep a per-epoch copy alongside latest.ckpt
                self.save_checkpoint(tag=f'{self.epoch}', use_thread=False)
    ...
    def step_ema(self):
        ...
        # EMAFlow only averages parameters. The obs encoder also carries
        # buffers (e.g. BatchNorm running stats), which would otherwise stay
        # frozen at their epoch_start_ema values, so copy them over.
        with torch.no_grad():
...
def main(cfg):
    workspace = TrainReFlowTransformerHybridWorkspace(cfg)
    workspace.run()
